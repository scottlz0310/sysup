...
2. **更新対象の選択** - 検出済みをデフォルト有効にし、不要なマネージャを無効化
3. **実行モードの選択** - 標準モード/自動実行モード
...
sysup
```
自動実行などでパスワード入力ができない場合は、`docs/AUTO_RUN.md`の「sudo権限の設定（特定コマンドのパスワード不要化）」も参照してください。
